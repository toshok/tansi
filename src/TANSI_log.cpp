...
// The log buffer is in special uninitialized RAM section so that it is not
// reset when soft rebooting or jumping from bootloader.
...
void log_raw(const std::string& str) { log_raw(str.c_str()); }

// Log byte as hex
void log_raw(uint8_t value) {
    const char* nibble = "0123456789ABCDEF";
    char hexbuf[5] = {'0', 'x', nibble[(value >> 4) & 0xF],
                      nibble[(value >> 0) & 0xF], 0};
    log_raw(hexbuf);
}
...
uint32_t log_get_buffer_len() { return g_logpos; }
...
const char* log_get_buffer(uint32_t* startpos, uint32_t* available) {
    ...
    if ((g_logpos & LOGBUFMASK) >= (*startpos & LOGBUFMASK)) {
        ...
        // Buffer wraps, read to end of buffer now and start from beginning on
        // next call.
