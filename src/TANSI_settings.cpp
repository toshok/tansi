...
void TANSISettings::setDefaultDriveInfo(uint8_t ansiId,
    ...
    if (!known_preset) {
        m_devPreset[ansiId] = DEV_PRESET_NONE;
        logmsg("Unknown Device preset name ", presetName,
               ", using default settings");
    }
}

// Read device settings
static void readIniANSIDeviceSettings(ansi_device_settings_t& cfg,
                                      const char* section) {
    cfg.bytesPerSector =
        inifile.getl(section, "BytesPerSector", cfg.bytesPerSector);
    cfg.sectorsPerTrack =
        inifile.getl(section, "SectorsPerTrack", cfg.sectorsPerTrack);
    cfg.headsPerCylinder =
        inifile.getl(section, "HeadsPerCylinder", cfg.headsPerCylinder);
    cfg.prefetchBytes =
        inifile.getl(section, "PrefetchBytes", cfg.prefetchBytes);

    cfg.sectorSDBegin =
        inifile.getl(section, "SectorSDBegin", cfg.sectorSDBegin);
...
ansi_system_settings_t* TANSISettings::initSystem(const char* presetName) {
    ...
    } else if (strequals(systemPresetName[SYS_PRESET_APOLLO_DN300],
                         presetName)) {
        m_sysPreset = SYS_PRESET_APOLLO_DN300;
        // cfgSys.quirks =
    } else {
        m_sysPreset = SYS_PRESET_NONE;
        logmsg("Unknown System preset name ", presetName,
               ", using default settings");
    }
...
ansi_system_settings_t* TANSISettings::getSystem() { return &m_sys; }

ansi_device_settings_t* TANSISettings::getDevice(uint8_t ansiId) {
    return &m_dev[ansiId];
}

ansi_system_preset_t TANSISettings::getSystemPreset() { return m_sysPreset; }
