...
void save_logfile(bool always = false) {
    ...
    if (loglen != prev_log_len && g_sdcard_present) {
        ...
        if (always || g_log_debug ||
            (LOG_SAVE_INTERVAL_MS > 0 &&
             (uint32_t)(millis() - prev_log_save) > LOG_SAVE_INTERVAL_MS)) {
...
void print_sd_info() {
    uint64_t size = (uint64_t)SD.sdfs.vol()->clusterCount() *
                    SD.sdfs.vol()->bytesPerCluster();
    ...
    if (SD.sdfs.card()->readCID(&sd_cid)) {
        logmsg("SD MID: ", (uint8_t)sd_cid.mid,
               ", OID: ", (uint8_t)sd_cid.oid[0], " ", (uint8_t)sd_cid.oid[1]);

        char sdname[6] = {sd_cid.pnm[0], sd_cid.pnm[1], sd_cid.pnm[2],
                          sd_cid.pnm[3], sd_cid.pnm[4], 0};
...
bool findHDDImages() {
    ...
    while (1) {
        ...
        bool is_hd = strlen(name) >= 2 &&
                     (tolower(name[0]) == 'h' && tolower(name[1]) == 'd');
        if (!is_hd) {
            continue;
        }
        ...
        if (idsSeen & (1 << id)) {
            logmsg("-- Ignoring ", fullname, ", ANSI ID ", id,
                   " is already in use!");
            continue;
        }
        ...
        if (g_ansi_settings.getDevicePreset(id) != DEV_PRESET_NONE) {
            logmsg("---- Using device preset: ",
                   g_ansi_settings.getDevicePresetName(id));
        }

        bool imageReady = ansiDiskOpenHDDImage(
        id, fullname,
        1056 /*XXX hardcoded apollo size.  should come from device settings*/);
        if (imageReady) {
            foundImage = true;
        } else {
...
static void reinitANSI() {
    ...
    ansiDiskResetImages();
    {
        ...
        if (ansiDiskCheckAnyImagesConfigured()) {
            // Ok, there is an image, turn LED on for the time it takes to
            // perform init
            LED_ON();
            delay(100);
        } else {
...
// Which is pretty much everything after platform_init and and
// platform_late_init
static void tansi_setup_sd_card() {
    ...
    if (g_sdcard_present) {
        ...
        char presetName[32];
        ini_gets("ANSI", "System", "", presetName, sizeof(presetName),
                 CONFIGFILE);
